...
public struct Moon {
    ...
    init(julianDay: Double, sun: Sun) {
        ...
        let coords = Coords(latitude: (Latitude0 * sin(meanDistance)).toRadians,
                            longitude: (Longitude0 * sin(meanAnomaly)).toRadians + eclipticalLongitude).equatorial
        let coordsRadians = coords.radians
        ...
        let angle = atan2(cos(sun.coords.latitude)
        ...
        let h = Earth.sidereal(julianDay: julianDay) - coordsRadians.longitude - coords.latitude
        phase = .init(inclination: inclination, angle: angle)
        fraction =  (1 + cos(inclination)) / 2
        altitude = Earth.astroRefraction(apparentAltitude: asin(
            sin(coordsRadians.latitude)
            * sin(coords.longitude)
            + cos(coordsRadians.latitude)
            * cos(coords.longitude)
            * cos(h)))
